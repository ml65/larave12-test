...
```bash
# Убедитесь, что в .env указаны настройки базы данных:
# DB_CONNECTION=mysql
# DB_HOST=mysql
# DB_PORT=3306
# DB_DATABASE=laravel
...
- Просматривать все доступные эндпойнты
- Тестировать API прямо из браузера
- Видеть примеры запросов и ответов
- Авторизоваться и использовать токен для защищенных эндпойнтов

Для обновления документации после изменений в коде выполните:
